...
interface PhotoDao {
    ...
    fun getAllBookmarksPagingSource(): PagingSource<Int, PhotoEntity>

    @Query("SELECT EXISTS(SELECT 1 FROM bookmarks WHERE id = :id)")
    fun isPhotoBookmarked(id: Int): Flow<Boolean>

    @Query("SELECT * FROM photos_cache ORDER BY timestamp DESC LIMIT 1")
    suspend fun getLatestCachedPhoto(): PhotoCacheEntity?
